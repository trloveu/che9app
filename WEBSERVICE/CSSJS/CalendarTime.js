//onfocus="SelectDate(this,this,false)" 

var bMoveable = true;
var strFrame;

document.writeln('<iframe id=endDateLayer frameborder=0 width=168 height=211 style="position: absolute;  z-index: 9998; display: none"></iframe>');
strFrame = '<style>';
...
strFrame += '        <td colspan=5 align=right style="color:#1478eb">&nbsp;&nbsp;<span onclick="parent.meizzToday()" style="font-size:12px;cursor: hand"';
strFrame += '         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="当前日期时间">今天</span>&nbsp;<span onclick="parent.setNull()" style="font-size:12px;cursor: hand"';
strFrame += '         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="将日期置空">置空</span>&nbsp;<span onclick="parent.setNullTT()" style="font-size:12px;cursor: hand"';
strFrame += '         onmouseover="style.color=\'#ff0000\'" onmouseout="style.color=\'#1478eb\'" title="关闭控件">关闭</span></td></tr>';
strFrame += '    </table></td></tr><tr ><td >';
strFrame += '        <table border=0 id=tbUseTime cellspacing=1 cellpadding=0 width=100%  bgcolor=#FFFFFF height=22 >';
strFrame += '          <tr bgcolor="#63A3E9"><td id=bUseTimeLayer width=30 align=center>';
strFrame += ' <span>时间</span></td>';
...
function SelectDate(tt, obj, bShowTime) {
        bUseTime = bShowTime;
        setday(tt, obj); 
} 
function setday(tt, obj) //主调函数
{
    ...
    //不显示时间 
    if (bUseTime == false) {
        odatelayer.tbUseTime.style.display = "none";
    }
    else {
        // bUseTime=true;
        // bImgSwitch();
        // odatelayer.bUseTimeLayer.innerHTML=bImg;
        odatelayer.tbUseTime.style.display = "";
...
function meizzToday()    //Today Button
{
    ...
    if (outObject) {
        if (bUseTime) {
            ...
            outObject.value = parent.meizzTheYear + "-" + format(parent.meizzTheMonth) + "-" +
                            format(parent.meizzTheDate); //注：在这里您可以输出改成您想要的格式
        }
    }
    closeLayer();
}
...
function WriteDateTo(yy, mm, n, hh, mi, se) {
    if (bUseTime) {
        outObject.value = yy + "-" + format(mm) + "-" + format(n) + " " + format(hh) + ":" + format(mi) + ":" + format(se); //注：在这里您可以输出改成您想要的格式
    }
    else {
        if (outObject.title == "TT") {
            var vDate = yy + "/" + format(mm) + "/" + format(n);
            var dateSelect = new Date(vDate);
            var dateSelect1 = (new Date((dateSelect / 1000 + 86400) * 1000))
            var CurrDate = new Date();
            if (dateSelect1 <= CurrDate) {
                alert("选择日期不能小于当前日期");
                outObject.value = "";
            }
            else
            ...
            outObject.value = yy + "-" + format(mm) + "-" + format(n); //注：在这里您可以输出改成您想要的格式
        }
    }
}
